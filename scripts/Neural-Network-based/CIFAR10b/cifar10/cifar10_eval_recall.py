...
tf.app.flags.DEFINE_string('eval_dir', '/tmp/cifar10_eval_barcodes',
                           """Directory where to write event logs.""")
tf.app.flags.DEFINE_string('eval_data', 'test',
                           """Either 'test' or 'train_eval'.""")
tf.app.flags.DEFINE_string('checkpoint_dir', '/tmp/cifar10_train_barcodes',
...
def eval_once(saver, summary_writer, top_k_op, summary_op, labels, logits):
  """Run Eval once.
  ...
  with tf.Session() as sess:
    ...
    try:
      ...
      for i in range(N):
          ...
          tp,tn,fp,fn = sess.run([true_positives,true_negatives, false_positives, false_negatives])
          #print(tp,tn,fp,fn)
          recall = float(tp) / float(tp+fp)
          precision = float(tp) / float(tp+fn)
          #print(recall, precision)
          cumr,cump = cumr+recall,cump+precision
          if i%10 == 0:
              print(i)
      print(cumr/float(N),cump/float(N))
...
def evaluate():
  ...
  with tf.Graph().as_default() as g:
    ...
    images, barcodes, labels = cifar10.inputs(eval_data=eval_data)

    # Build a Graph that computes the logits predictions from the
    # inference model.
    logits = cifar10.inference(images,barcodes)
    ...
    while True:
      eval_once(saver, summary_writer, top_k_op, summary_op, labels, logits)
      if FLAGS.run_once:
        break
