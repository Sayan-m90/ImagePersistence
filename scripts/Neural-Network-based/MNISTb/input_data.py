...
def read_barcodes(f):
  with open(f,'r') as bc_file:
      res = []
      text = bc_file.read()
      for line in text.split('\n')[:-1]:
          dim = map(float,line[1:-1].split(','))
          mdim = max(dim+[1])
          dim = [d/float(mdim) for d in dim]
          for d in dim:
              res.append(d)
  # res[0]=1
  return res

def extract_barcodes(FILEPATH):
  paths = glob.glob(FILEPATH+'*.dat')
  print(len(paths))
  nums = []
  for p in paths:
      nums.append(int(p.split('/')[-1].split('_')[0]))

  nums,paths = zip(*sorted(zip(nums,paths)))
  data = []
  for p in paths:
      bc_path = p.split('.')[0]+'/barcodes'
...
class DataSet(object):
  ...
  def next_batch(self, batch_size, fake_data=False, shuffle=True):
    ...
    if self._epochs_completed == 0 and start == 0 and shuffle:
      ...
      self._barcodes = self.barcodes[perm0]
    ...
    if start + batch_size > self._num_examples:
      ...
      barcodes_rest_part = self._barcodes[start:self._num_examples]
      # Shuffle the data
      if shuffle:
        ...
        self._barcodes = self.barcodes[perm]
      # Start next epoch
      start = 0
      ...
      barcodes_new_part = self.barcodes[start:end]
      return numpy.concatenate((images_rest_part, images_new_part), axis=0) , numpy.concatenate((labels_rest_part, labels_new_part), axis=0), numpy.concatenate((barcodes_rest_part, barcodes_new_part), axis=0)
    else:
      self._index_in_epoch += batch_size
      end = self._index_in_epoch
      return self._images[start:end], self._labels[start:end], self._barcodes[start:end]
